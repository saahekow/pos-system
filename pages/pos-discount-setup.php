...
$pageTitle='Plug Discount Setup';
$breadcrumbs=[['label'=>'Home','url'=>app_url('index.php')],['label'=>'Admin','url'=>app_url('admin.php')],['label'=>'Setup','url'=>app_url('setup.php')],['label'=>'Plug Discounts']];
...
if($editId){$edit=db()->prepare('SELECT d.*,sp.brand_name FROM plug_discounts d INNER JOIN spark_plugs sp ON sp.id=d.spark_plug_id WHERE d.id=?');$edit->execute([$editId]);if($row=$edit->fetch())$form=['brand_name'=>(string)$row['brand_name'],'spark_plug_id'=>(string)$row['spark_plug_id'],'discount_percentage'=>(string)$row['discount_percentage'],'is_active'=>(string)(int)$row['is_active']];else $editId=0;}
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!verify_csrf_token((string)($_POST['csrf_token']??'')))$error='Your session expired. Please try again.';
        ...
        elseif($percentage<0||$percentage>100)$error='Discount percentage must be between 0 and 100.';
        else{
            try{
                if($editId){db()->prepare('UPDATE plug_discounts SET spark_plug_id=?,discount_percentage=?,is_active=? WHERE id=?')->execute([(int)$form['spark_plug_id'],$percentage,$form['is_active']==='0'?0:1,$editId]);$message='Plug discount updated successfully.';}
                else{db()->prepare('INSERT INTO plug_discounts(spark_plug_id,discount_percentage,is_active,created_by_user_id) VALUES(?,?,?,?)')->execute([(int)$form['spark_plug_id'],$percentage,$form['is_active']==='0'?0:1,current_user_id()]);$message='Plug discount added successfully.';}
                $editId=0;$form=['brand_name'=>'','spark_plug_id'=>'','discount_percentage'=>'20.00','is_active'=>'1'];
            }catch(PDOException $exception){$error=(string)$exception->getCode()==='23000'?'That plug number already has a discount. Edit its existing discount instead.':'The discount could not be saved.';}
        }
    }
}
...
$discounts=db()->query("SELECT d.id,d.discount_percentage,d.is_active,d.created_at,sp.brand_name,sp.plug_number FROM plug_discounts d INNER JOIN spark_plugs sp ON sp.id=d.spark_plug_id ORDER BY sp.brand_name,sp.plug_number")->fetchAll();
require_once __DIR__ . '/../includes/header.php';
?>
<section class="management-panel"><div class="management-heading"><div><span class="section-kicker">POS Setup</span><h1>Plug Discounts</h1><p>Assign the percentage discount applied to each complete box of a plug number.</p></div><div class="management-icon"><i class="fa-solid fa-percent"></i></div></div>
