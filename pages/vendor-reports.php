...
$customerTownStatement = db()->prepare(
    "SELECT DISTINCT l.id,l.id AS location_id,l.town_name,l.is_capital,l.region_code,l.region_name,l.mmda_name,l.mmda_name AS district_name
     ...
     WHERE v.record_status='completed' AND v.deleted_at IS NULL AND v.visit_type='registration'
...
$normalizedVisitStatement = db()->prepare(
    "SELECT v.id,v.customer_id,c.customer_name,c.customer_name AS contact_name,c.phone,c.other_phone,
            ...
            v.recorded_by_user_id AS created_by_user_id,v.created_at,NULL AS sales_ref,
            0 AS sale_confirmed,'normalized_visit' AS sales_source,
     ...
     LEFT JOIN customer_promo_plugs cs ON cs.id=(
          SELECT cs_latest.id
          FROM customer_promo_plugs cs_latest
...
?>
<?php $recycleReturnUrl=safe_app_return_url((string)($_SERVER['REQUEST_URI']??''),app_url('vendor-reports.php')); ?>
<div class="report-bin-shortcut"><a href="<?=e(app_url('recycle-bin.php?module=vendor&section=customers&return_to='.rawurlencode($recycleReturnUrl)))?>" title="Open Recycle Bin"><i class="fa-solid fa-trash-can"></i><span>Recycle Bin</span></a></div>
...
<section class="management-panel">
    ...
    <div class="report-destination-grid"><a class="report-destination-card" href="<?= e(app_url('vendor-reports.php?report=customers')) ?>"><span class="report-destination-card__icon"><i class="fa-solid fa-users"></i></span><strong>Customers</strong><span>Open reports <i class="fa-solid fa-arrow-right"></i></span></a><a class="report-destination-card" href="<?=e(app_url('recycle-bin.php?module=vendor&section=customers'))?>"><span class="report-destination-card__icon"><i class="fa-solid fa-recycle"></i></span><strong>Recycle Bin</strong><span>Review deleted vendor records <i class="fa-solid fa-arrow-right"></i></span></a></div>
</section>
...
<section class="management-panel management-panel--table" data-report-listing>
    ...
    <form class="report-filter-form" data-report-filter-form data-report-require-filter>
        <?php if ($mode === 'type'): ?>
        ...
        <div class="report-filter-panel">
            ...
            <div class="form-field report-date-field"><label for="date_from">Date From</label><input id="date_from" name="date_from" type="date" value="<?=e(date('Y-m-d'))?>" data-report-date-filter></div>
            <div class="form-field report-date-field"><label for="date_to">Date To</label><input id="date_to" name="date_to" type="date" value="<?=e(date('Y-m-d'))?>" data-report-date-filter></div>
        </div>
        <?php endif; ?>
